//Variables, Data Types, and Typing

let wordData = "Word Data";
console.log(wordData);
//
let subTotal = 100;
let tax = subTotal * 0.07;
let totalWithTax = tax + subTotal;

/*Data Types:
used to describe the kind of data in code specific to the language used:
JS has... 5 Primitive data types:
boolean
null
undefined
number- often broken up into separate types in other languages: int (whole), float (dec), double.
strings
Composite data types:
object - by creating a variable
*/
let obj = {
	key1: "value 1",
	key2: 3,
	boolean: true,
	obj2: {
		obj2Key1 : "value of inner object",
	}
};

console.log(obj.obj2.obj2Key1);
//
let var1 = "2";
let var2 = 5;
let result = var1 +var2;

console.log(result);
//
let add = 1 + 2;
let sub = 2 - 1;
let mult = 2*4;
let division = 4 / 2;
let mod = 5 % 2;

let string = "string 1 " + "string 2";

console.log(string);